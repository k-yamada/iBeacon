...
//  Beacon
//
//  Created by 山田 和弘 on 2015/04/13.
//  Copyright (c) 2015年 山田 和弘. All rights reserved.
//
...
class ViewController: UIViewController, CBPeripheralManagerDelegate, UIPickerViewDelegate, UIPickerViewDataSource , UITextFieldDelegate{
  ...
  var myMajorId1: String = "0"
  
  // MajorId(下位)
  
  var myMajorId2: String = "0"
  
  // MinorId(上位)
  
  var myMinorId1: String = "0"
  
  // MinorId(下位)
  
  var myMinorId2: String = "0"
  ...
  */
  func onClickMyButton(sender: UIButton){
    ...
    if sender == myWindowButton {
      ...
      // iBeaconのUUID.
      let myProximityUUID = NSUUID(UUIDString: myUuid as String)
      
      // iBeaconのIdentifier.
      let myIdentifier = "akabeacon"
      
      // MajorId
      let myMajorString: String = myMajorId1 + myMajorId2
      ...
      // MinorId
      let myMinorString: String = myMinorId1 + myMinorId2
      ...
      let myBeaconPeripheralData: NSDictionary = myBeaconRegion.peripheralDataWithMeasuredPower(nil)
      
      // Advertisingを発信.
      //myPheripheralManager.startAdvertising(myBeaconPeripheralData)
      myPheripheralManager.startAdvertising([CBAdvertisementDataServiceUUIDsKey:[myUuid]])
    }
